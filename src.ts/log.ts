...
function _getPrompt(prompt: string, options: PromptOptions, callback: (ctrlC: boolean, message: null | string) => void) {
    ...
    let respond = (ctrlC: boolean, message: null | string) => {
        process.stdout.write('\n');
            ...
            case "\u0004":
                if (options.choice) {
                    if (options.defaultChoice) {
                        respond(false, options.defaultChoice);
                    }
                } else {
